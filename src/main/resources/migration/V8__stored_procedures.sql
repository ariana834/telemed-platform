-- Functions:
--   1. generate_medical_form – generates personalized form based on symptoms + history
--   2. compute_preliminary_diagnosis – scores symptoms to produce a ranked diagnosis
--   3. schedule_next_appointment – finds the first available doctor slot (7-day window)
--   4. auto_generate_prescription – issues OTC-only prescription for simple cases
...
v_has_fever        := v_symptoms_text ILIKE '%fever%'
                       OR v_symptoms_text ILIKE '%temperature%'
                       OR v_symptoms_text ILIKE '%high temp%';

    v_has_abdominal    := v_symptoms_text ILIKE '%abdomin%'
                       OR v_symptoms_text ILIKE '%stomach%'
                       OR v_symptoms_text ILIKE '%belly%'
                       OR v_symptoms_text ILIKE '%abdomen%';

    v_has_headache     := v_symptoms_text ILIKE '%headache%'
                       OR v_symptoms_text ILIKE '%head pain%'
                       OR v_symptoms_text ILIKE '%migraine%';

    v_has_vomiting     := v_symptoms_text ILIKE '%vomit%'
                       OR v_symptoms_text ILIKE '%nausea%'
                       OR v_symptoms_text ILIKE '%throwing up%';

    v_has_no_appetite  := v_symptoms_text ILIKE '%appetite%'
                       OR v_symptoms_text ILIKE '%no appetite%'
                       OR v_symptoms_text ILIKE '%not eating%';

    v_has_cough        := v_symptoms_text ILIKE '%cough%'
                       OR v_symptoms_text ILIKE '%coughing%';

    v_has_chest_pain   := v_symptoms_text ILIKE '%chest%'
                       OR v_symptoms_text ILIKE '%chest pain%'
                       OR v_symptoms_text ILIKE '%chest tightness%';

    v_has_breathing    := v_symptoms_text ILIKE '%breath%'
                       OR v_symptoms_text ILIKE '%shortness of breath%'
                       OR v_symptoms_text ILIKE '%dyspnea%'
                       OR v_symptoms_text ILIKE '%breathing%';

    v_has_fatigue      := v_symptoms_text ILIKE '%fatigue%'
                       OR v_symptoms_text ILIKE '%tired%'
                       OR v_symptoms_text ILIKE '%exhausted%'
                       OR v_symptoms_text ILIKE '%weakness%';

    v_has_rash         := v_symptoms_text ILIKE '%rash%'
                       OR v_symptoms_text ILIKE '%spots%'
                       OR v_symptoms_text ILIKE '%hives%'
                       OR v_symptoms_text ILIKE '%skin redness%';

    v_has_sore_throat  := v_symptoms_text ILIKE '%throat%'
                       OR v_symptoms_text ILIKE '%sore throat%'
                       OR v_symptoms_text ILIKE '%swallowing%';

    v_has_ear_pain     := v_symptoms_text ILIKE '%ear%'
                       OR v_symptoms_text ILIKE '%ear pain%'
                       OR v_symptoms_text ILIKE '%earache%';


SELECT severity INTO v_abdominal_severity
FROM   consultation_symptoms
WHERE  consultation_id = p_consultation_id
  AND  (LOWER(symptom_name) ILIKE '%abdomin%'
            OR LOWER(symptom_name) ILIKE '%stomach%'
            OR LOWER(symptom_name) ILIKE '%belly%')
    LIMIT 1;


SELECT
    BOOL_OR(LOWER(condition_name) ILIKE '%diabet%'),
    BOOL_OR(LOWER(condition_name) ILIKE '%hypertension%'
             OR LOWER(condition_name) ILIKE '%high blood pressure%'),
    BOOL_OR(LOWER(condition_name) ILIKE '%heart%'
             OR LOWER(condition_name) ILIKE '%cardiac%'),
    BOOL_OR(LOWER(condition_name) ILIKE '%digestive%'
             OR LOWER(condition_name) ILIKE '%gastritis%'
             OR LOWER(condition_name) ILIKE '%gastric%'),
    BOOL_OR(LOWER(condition_name) ILIKE '%allerg%')
INTO v_has_diabetes, v_has_hypertension, v_has_heart_disease,
    v_has_digestive, v_has_allergies
...
VALUES
    (p_consultation_id,
     'Is the pain localized in the lower right side of the abdomen?',
     'YES_NO', NULL, 1, TRUE),
    (p_consultation_id,
     'How long have the symptoms been present?',
     'MULTIPLE_CHOICE',
     '["Less than 1 hour", "1–6 hours", "6–12 hours", "More than 12 hours"]',
     2, TRUE),
    (p_consultation_id,
     'Has the pain progressively worsened since it started?',
     'YES_NO', NULL, 3, TRUE),
    (p_consultation_id,
     'Do you have a fever?',
     'YES_NO', NULL, 4, TRUE);

RAISE NOTICE 'EMERGENCY_REDIRECT: Consultation % redirected to emergency (possible appendicitis)',
            p_consultation_id;
        RETURN;
END IF;
INSERT INTO medical_form_questions
...
VALUES
    (p_consultation_id,
     ...
     'MULTIPLE_CHOICE',
     '["1 day", "2–3 days", "4–7 days", "More than a week"]',
     v_order_idx, TRUE);
...
VALUES
    (p_consultation_id,
     'How would you rate the overall intensity of your symptoms?',
     'MULTIPLE_CHOICE',
     '["Mild — I can carry out daily activities normally", "Moderate — activities are affected", "Severe — I cannot carry out daily activities"]',
     v_order_idx, TRUE);
v_order_idx := v_order_idx + 1;
    IF v_has_chest_pain AND v_has_breathing THEN
        ...
        VALUES
            (p_consultation_id,
             ...
             'MULTIPLE_CHOICE',
             '["Dull / pressure-like", "Sharp / stabbing", "Burning", "Tightness"]',
             v_order_idx, TRUE),
            (p_consultation_id,
             'Does the pain radiate to your arm, jaw, or back?',
             'YES_NO', NULL, v_order_idx + 1, TRUE),
            (p_consultation_id,
             'Does the shortness of breath occur at rest or only during exertion?',
             'MULTIPLE_CHOICE',
             '["At rest", "During light exertion", "Only during intense exertion"]',
             v_order_idx + 2, TRUE),
            (p_consultation_id,
             'Have you experienced palpitations, dizziness, or fainting?',
             'CHECKBOX',
             '["Palpitations", "Dizziness", "Fainting", "None of the above"]',
             v_order_idx + 3, TRUE),
            (p_consultation_id,
             'Have you had a recent respiratory infection (cold, flu, COVID-19)?',
             'YES_NO', NULL, v_order_idx + 4, FALSE);
        v_order_idx := v_order_idx + 5;

        -- extra questions for patients with known cardiac or hypertension history
        IF v_has_heart_disease OR v_has_hypertension THEN
            INSERT INTO medical_form_questions
                (consultation_id, question_text, question_type, options, order_index, is_required)
            VALUES
                (p_consultation_id,
                 'What was your last recorded blood pressure reading?',
                 'OPEN_TEXT', NULL, v_order_idx, TRUE),
                (p_consultation_id,
                 'Are you currently taking your prescribed cardiac / blood pressure medication?',
                 'YES_NO', NULL, v_order_idx + 1, TRUE);
    ...
    IF v_has_fever AND v_has_abdominal AND v_has_vomiting THEN
        ...
        VALUES
            (p_consultation_id,
             ...
             'MULTIPLE_CHOICE',
             '["37–37.5 °C (low-grade)", "37.5–38.5 °C (moderate)", "38.5–39.5 °C (high)", "Above 39.5 °C"]',
             v_order_idx, TRUE),
            (p_consultation_id,
             'Did you consume potentially spoiled food in the last 24 hours?',
             'YES_NO', NULL, v_order_idx + 1, TRUE),
            (p_consultation_id,
             'Do you have diarrhoea?',
             'YES_NO', NULL, v_order_idx + 2, TRUE),
            (p_consultation_id,
             'Are other people around you experiencing the same symptoms?',
             'YES_NO', NULL, v_order_idx + 3, TRUE),
            (p_consultation_id,
             'Is vomiting frequent (more than 3 episodes per day)?',
             'YES_NO', NULL, v_order_idx + 4, FALSE);
        v_order_idx := v_order_idx + 5;
        ...
        IF v_has_digestive THEN
            ...
            VALUES
                (p_consultation_id,
                 'Do the current symptoms resemble previous episodes of your chronic digestive condition?',
                 'YES_NO', NULL, v_order_idx, FALSE),
                (p_consultation_id,
                 'Are you currently following a treatment plan for your digestive condition?',
                 'YES_NO', NULL, v_order_idx + 1, TRUE);
            v_order_idx := v_order_idx + 2;
END IF;
    ...
    IF v_has_fever AND v_has_headache AND (v_has_cough OR v_has_fatigue) THEN
        ...
        VALUES
            ...
            (p_consultation_id,
             ...
             'YES_NO', NULL, v_order_idx + 2, TRUE),
            (p_consultation_id,
             'Which nasal symptoms are you experiencing?',
             'CHECKBOX',
             '["Blocked nose", "Clear nasal discharge", "Yellow/green nasal discharge", "None"]',
             v_order_idx + 3, FALSE),
            (p_consultation_id,
             'Have you been vaccinated against influenza this season?',
             'YES_NO', NULL, v_order_idx + 4, FALSE);
        v_order_idx := v_order_idx + 5;
        ...
        IF v_has_hypertension THEN
            ...
            VALUES
                (p_consultation_id,
                 'Have you measured your blood pressure recently? Please share the reading.',
                 'OPEN_TEXT', NULL, v_order_idx, TRUE);
            v_order_idx := v_order_idx + 1;
END IF;
...
END IF;
    ...
    IF v_is_child AND v_has_fever AND (v_has_vomiting OR v_has_no_appetite) THEN
        ...
        VALUES
            (p_consultation_id,
             'Does the child have any skin rash or spots?',
             'YES_NO', NULL, v_order_idx, TRUE),
            (p_consultation_id,
             'Is the child pulling at their ears or complaining of ear pain?',
             'YES_NO', NULL, v_order_idx + 1, TRUE),
            (p_consultation_id,
             'What is the child''s current temperature?',
             'MULTIPLE_CHOICE',
             '["37–37.5 °C", "37.5–38.5 °C", "38.5–39.5 °C", "Above 39.5 °C"]',
             v_order_idx + 2, TRUE),
            (p_consultation_id,
             'Has the child been in contact with other ill children in the past 7 days?',
             'YES_NO', NULL, v_order_idx + 3, TRUE),
            (p_consultation_id,
             'Is the child''s vaccination schedule up to date?',
             'YES_NO', NULL, v_order_idx + 4, FALSE),
            (p_consultation_id,
             'Is the child having difficulty breathing or breathing faster than normal?',
...
VALUES
    ...
    (p_consultation_id,
     ...
     'YES_NO', NULL, v_order_idx + 2, TRUE),
    (p_consultation_id,
     'Please briefly describe how your symptoms have evolved since they started.',
...
v_order_idx := v_order_idx + 4;
    ...
    IF v_has_diabetes THEN
        ...
        VALUES
            (p_consultation_id,
             'Have you monitored your blood glucose in the last 24 hours? Please share the readings.',
             'OPEN_TEXT', NULL, v_order_idx, TRUE);
        v_order_idx := v_order_idx + 1;
END IF;
    ...
    IF v_has_allergies THEN
        ...
        VALUES
            (p_consultation_id,
             'Have you recently been exposed to known allergens (dust, pollen, food, medication)?',
             'YES_NO', NULL, v_order_idx, TRUE);
        v_order_idx := v_order_idx + 1;
END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'FORM_GENERATION_ERROR: Failed to generate form for consultation %: %',
...
v_patient_id            BIGINT;
    ...
    -- derived booleans from form answers
...
-- emergency: insert a direct diagnosis, skip scoring
IF v_complexity = 'EMERGENCY' THEN
        ...
        VALUES
            (p_consultation_id,
             'Possible Acute Appendicitis — requires emergency surgical evaluation',
             'AUTO_GENERATED', 'EMERGENCY', 'K37', 75,
             'Symptoms indicate a possible surgical emergency. Patient redirected to the nearest ER.');
        RETURN;
END IF;
...
SELECT
    BOOL_OR(LOWER(a.answer_text) IN ('yes') AND q.question_text ILIKE '%spoiled food%'),
    BOOL_OR(LOWER(a.answer_text) IN ('yes') AND q.question_text ILIKE '%other people%'),
    BOOL_OR(a.answer_text ILIKE '%39%'
             OR a.answer_text ILIKE '%40%'
             OR a.answer_text ILIKE '%above 39%'
             OR a.answer_text ILIKE '%high%'),
...
INTO  v_bad_food, v_others_sick, v_high_fever,
    v_muscle_pain, v_ear_pain, v_has_rash,
    v_has_diarrhea, v_pain_radiates, v_has_palpitations
...
IF v_symptoms_text ILIKE '%fever%'    THEN v_score_flu := v_score_flu + 15; END IF;
    ...
    IF v_symptoms_text ILIKE '%fever%' AND NOT v_high_fever
                                           THEN v_score_cold := v_score_cold + 25; END IF;
    IF v_symptoms_text ILIKE '%cough%'    THEN v_score_cold := v_score_cold + 20; END IF;
    IF v_symptoms_text ILIKE '%nasal%'
    OR v_symptoms_text ILIKE '%runny%'    THEN v_score_cold := v_score_cold + 15; END IF;
    ...
    IF v_symptoms_text ILIKE '%vomit%'
    OR v_symptoms_text ILIKE '%nausea%'   THEN v_score_food_poisoning := v_score_food_poisoning + 20; END IF;
    ...
    IF v_symptoms_text ILIKE '%stomach%'
    OR v_symptoms_text ILIKE '%abdom%'    THEN v_score_gastroenteritis := v_score_gastroenteritis + 20; END IF;
    IF v_symptoms_text ILIKE '%vomit%'
    OR v_symptoms_text ILIKE '%nausea%'   THEN v_score_gastroenteritis := v_score_gastroenteritis + 20; END IF;
    IF v_symptoms_text ILIKE '%fever%' AND NOT v_high_fever
                                           THEN v_score_gastroenteritis := v_score_gastroenteritis + 15; END IF;
...
-- primary diagnosis (highest score above threshold)
...
-- secondary diagnosis (second highest, different from primary)
SELECT diag_name, icd, score
...
END IF;
    ...
    -- fallback: no condition scored above threshold
    IF v_diagnosis1 IS NULL THEN
        INSERT INTO diagnoses
            (consultation_id, diagnosis_name, diagnosis_type, complexity_level, confidence_score, notes)
        VALUES
            (p_consultation_id,
             'Unspecified condition — medical evaluation required',
             'AUTO_GENERATED', 'MEDIUM', 30,
             'Symptoms do not match a clear diagnostic pattern. A consultation with a doctor is recommended.');
END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'DIAGNOSIS_ERROR: Failed to compute diagnosis for consultation %: %',
            p_consultation_id, SQLERRM;
END;
$$ LANGUAGE plpgsql;
...
END;
    ...
    -- search window: next full hour → +7 days
    v_search_from  := DATE_TRUNC('hour', NOW()) + INTERVAL '1 hour';
    v_search_limit := v_search_from + INTERVAL '7 days';

    -- iterate in 10-minute increments until a free slot is found
...
ORDER  BY d.id
    LOOP
    -- check for overlapping appointments using PostgreSQL range operator
    IF NOT EXISTS (
    SELECT 1 FROM appointments
    WHERE  doctor_id = r_doctor.doctor_id
    AND  status NOT IN ('CANCELLED', 'NO_SHOW')
    AND  tstzrange(start_time, end_time) &&
    tstzrange(v_search_from,
    v_search_from + (v_duration || ' minutes')::INTERVAL)
    ) THEN
...
END LOOP;
    ...
    IF NOT v_found THEN
        RAISE EXCEPTION 'NO_SLOTS_AVAILABLE: No available slot in the next 7 days for complexity %',
            v_complexity;
END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'SCHEDULING_ERROR: Failed to schedule consultation %: %',
            p_consultation_id, SQLERRM;
END;
$$ LANGUAGE plpgsql;
...
-- ================================================================
-- 4. AUTOMATIC PRESCRIPTION GENERATION (OTC only, no antibiotics)
-- ================================================================
...
v_patient_id        BIGINT;
    ...
    -- only these diagnoses are eligible for automatic OTC prescriptions
...
IF v_complexity != 'SIMPLE' THEN
        RAISE EXCEPTION
            'AUTO_PRESCRIPTION_NOT_ALLOWED: Automatic prescriptions are only available for simple cases. Current complexity: %',
            v_complexity;
END IF;

    -- pick the highest-confidence auto-generated diagnosis
...
IF NOT (v_diagnosis_name = ANY(v_eligible_diagnoses)) THEN
        RAISE EXCEPTION
            'DIAGNOSIS_NOT_ELIGIBLE: Diagnosis "%" does not qualify for an automatic prescription. The patient will be scheduled with a doctor.',
            v_diagnosis_name;
END IF;
...
IF v_diagnosis_name IN ('Upper Respiratory Tract Infection (Cold)', 'Influenza (Flu)') THEN
        ...
        VALUES
            (v_prescription_id,
             'Paracetamol 500 mg', '1 tablet', 'Every 6–8 hours as needed', 5,
             'Do not exceed 4 tablets per day. Take with food or water.'),
            (v_prescription_id,
             'Vitamin C 1000 mg (effervescent)', '1 tablet', 'Once daily', 7,
             'Dissolve in a glass of water. Take after meals.'),
            (v_prescription_id,
             'Ibuprofen 400 mg', '1 tablet', 'Every 8 hours as needed', 3,
             'Take with food. Contraindicated if you have a peptic ulcer or kidney issues.');
END IF;

    -- ── Food Poisoning / Gastroenteritis medications ─────────────
    IF v_diagnosis_name IN ('Food Poisoning', 'Acute Gastroenteritis') THEN
        INSERT INTO prescription_medications
            (prescription_id, medication_name, dosage, frequency, duration_days, instructions)
        VALUES
            (v_prescription_id,
             'Diosmectite (Smecta)', '1 sachet', 'Three times daily', 3,
             'Dissolve in 100 ml of water. Take between meals.'),
            (v_prescription_id,
             'Oral Rehydration Solution', '200–400 ml',
             'After each episode of vomiting or diarrhoea', 3,
             'Rehydration is essential. Sip slowly — do not drink all at once.'),
            (v_prescription_id,
             'Paracetamol 500 mg', '1 tablet', 'Every 6–8 hours as needed', 3,
             'Only if you have a fever. Do not exceed 4 tablets per day.');
END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'PRESCRIPTION_ERROR: Failed to generate prescription for consultation %: %',
            p_consultation_id, SQLERRM;
END;
$$ LANGUAGE plpgsql;